...
#define POT_PIN 26
#define L_PWM 14
#define R_PWM 15 // Este pin se usará para calibrar el Talon
...
//-------------------- INICIALIZACION PWM LED---------------------------
    gpio_set_function(L_PWM, GPIO_FUNC_PWM); // seleccionamos la funcion de gpio con el 
    //numero de pin correspondiente a LED_PIN, la función será PWM
    
//-------------------- INICIALIZACION PWM TALON----------------------------------------
    gpio_set_function(R_PWM, GPIO_FUNC_PWM); // seleccionamos la funcion de gpio con el 
    ...
    uint slice_num_A = pwm_gpio_to_slice_num(L_PWM);
    uint slice_num_B = pwm_gpio_to_slice_num(R_PWM);
    ...
    pwm_set_chan_level(slice_num_A, PWM_CHAN_A,0); //Empezamos en 0 el nivel del PWM
    //RECORDEMOS QUE ESTAMOS EN EL CANAL A
    pwm_set_chan_level(slice_num_A, PWM_CHAN_B,0); //Empezamos en 0 el nivel del PWM
    //RECORDEMOS QUE ESTAMOS EN EL CANAL B
    pwm_set_chan_level(slice_num_B, PWM_CHAN_A,0); //Empezamos en 0 el nivel del PWM
    //RECORDEMOS QUE ESTAMOS EN EL CANAL A
    pwm_set_chan_level(slice_num_B, PWM_CHAN_B,0); //Empezamos en 0 el nivel del PWM
    //RECORDEMOS QUE ESTAMOS EN EL CANAL B

    pwm_set_enabled(slice_num_A,true); //Habilitamos el PWM
    pwm_set_enabled(slice_num_B,true); //Habilitamos el PWM
...
//loop principal
    while (1)
    {
        ...
        if(voltaje == 1.5 ){
	 pwm_set_chan_level(slice_num_A, PWM_CHAN_A, 256);
         pwm_set_chan_level(slice_num_B, PWM_CHAN_B, 256);
	printf("Alto \t Raw: %d \t PWM: %d \t Voltaje PWM: %.1f \n", result, pwm_value, voltaje);
	}
	if(voltaje <= 1.4){
        ...
        //INDICAMOS QUE USAMOS EL VALOR DEL PWM DEL POTENCIOMETRO
         ...
         pwm_set_chan_level(slice_num_B, PWM_CHAN_B, 256);
         printf("Izquieda \t Raw: %d \t PWM: %d \t Voltaje PWM: %.1f \n", result, pwm_value, voltaje);
        }
	if(voltaje >= 1.6){
	//INDICAMOS QUE USAREMOS EL CANAL B
        //INDICAMOS QUE USAMOS EL VALOR DEL PWM DEL POTENCIOMETRO
         ...
         printf("Derecha \t Raw: %d \t PWM: %d \t Voltaje PWM: %.1f \n", result, pwm_value, voltaje);
         }
